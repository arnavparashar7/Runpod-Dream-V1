...
# --- Import the process_handler ---
from process_handler import handler as process_job_input_handler
# ----------------------------------
...
# --- Cloudflare Configuration (Add these to your environment variables) ---
CLOUDFLARE_ACCOUNT_ID = os.environ.get("CLOUDFLARE_ACCOUNT_ID")
CLOUDFLARE_API_TOKEN = os.environ.get("CLOUDFLARE_API_TOKEN")
# --------------------------------------------------------------------------
...
def validate_input(job_input):
    ...
    """
    # This validation will now apply to the *processed* input from process_handler
    # It still needs to check for 'workflow' and 'images' as the primary keys.
...
def handler(job):
    ...
    This handler first preprocesses the input using process_handler.py.

    Args:
        job (dict): A dictionary containing job details and input parameters
                    (expected to be in the format for process_handler).
    ...
    # --- Step 1: Preprocess the input using process_handler ---
    ...
    try:
        ...
        return {"error": f"Failed to preprocess input: {e}"}

    # Check if process_handler returned an error
    if runpod_input and "error" in runpod_input:
        print(
            f"worker-comfyui - Preprocessing returned an error: {runpod_input['error']}"
        )
        return runpod_input  # Pass the error directly back

    # Now, the 'job_input' for the rest of this handler is the 'runpod_input'
    workflow = runpod_input.get("workflow")
    input_images = runpod_input.get("images")
    ...
    try:
                ...
                if isinstance(out, str):
                    ...
                    if message.get("type") == "status":
                        ...
                        if (
                            ...
                            # You might want to log current node execution for more detailed progress
            ...
            if "images" in node_output:
                ...
                for image_info in node_output["images"]:
                    ...
                    if image_bytes:
                        ...
                        # --- NEW: Cloudflare Upload Logic ---
                        if CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN:
                            print(f"worker-comfyui - Uploading {filename} to Cloudflare...")
                            try:
                                api_url = f"https://api.cloudflare.com/client/v4/accounts/{CLOUDFLARE_ACCOUNT_ID}/images/v1"
                                headers = {"Authorization": f"Bearer {CLOUDFLARE_API_TOKEN}"}
                                # Generate a unique ID for the filename to avoid collisions,
                                # ensuring it's always a PNG for Cloudflare if original extension is missing.
                                # ComfyUI outputs are generally PNG or JPEG, so we assume PNG for default.
                                cloudflare_filename = f"{uuid.uuid4()}{file_extension}"
                                files = {"file": (cloudflare_filename, image_bytes, "image/png")}
                                ...
                                output_data.append(
                                    {
                                        "filename": filename, # Original filename from ComfyUI
                                        "type": "cloudflare_url",
                                        "data": public_url,
                                    }
                                )
                                print(f"worker-comfyui - Uploaded {filename} to Cloudflare: {public_url}")
                            except requests.RequestException as e:
                                error_msg = f"Error uploading {filename} to Cloudflare: {e}"
                                print(f"worker-comfyui - {error_msg}")
                                errors.append(error_msg)
                            except Exception as e:
                                error_msg = f"Unexpected error with Cloudflare upload for {filename}: {e}"
                                print(f"worker-comfyui - {error_msg}")
                                errors.append(error_msg)

                        # --- Existing: S3 Upload Logic (if Cloudflare not configured) ---
                        ...
                        # --- Fallback: Return as base64 string if no external upload configured ---
                        else:
        ...
        print(f"worker-comfyui - Workflow Validation/Queueing Error: {e}")
        print(traceback.format_exc())
        errors.append(f"Workflow processing error: {e}")
    except Exception as e:
        print(f"worker-comfyui - An unexpected error occurred: {e}")
        print(traceback.format_exc())
        errors.append(f"An unexpected error occurred: {e}")
    finally:
        if ws:
            ws.close()
            print("worker-comfyui - Websocket connection closed.")
    ...
    if errors:
        return {
            ...
            "details": errors,
            "output": output_data,  # Still return any images that were successfully generated
        }
    elif not output_data:
        return {
            "error": "Job completed successfully but no output images were found.",
            "details": ["No image outputs detected in ComfyUI history."],
        }
    else:
